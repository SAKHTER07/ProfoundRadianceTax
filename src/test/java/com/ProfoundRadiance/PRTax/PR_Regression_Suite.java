...
public class PR_Regression_Suite {
  ...
  public void PRuserInOfficeAppointment() throws InterruptedException {
     ...
     driver.findElement(By.id("filing_case_appointment_date")).sendKeys("07/06/2021");
  ...
  for(int i=0; i<appmntTimes.size(); i++)
  {
   ...
   String availableAppmntTime = availableAppmntTimes.getAttribute("value");
   System.out.println("List of available appointment times are=======>>>>>"+availableAppmntTime);
   Thread.sleep(4000); 
   
   if(availableAppmntTime.equals(appmntTimes)) {
